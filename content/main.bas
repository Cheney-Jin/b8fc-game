...
REM Resources.
equipment = load_resource("equipment.sprite")
...
spaceship = load_resource("spaceship_1.sprite")
spaceship.play()
		...
		if x < -16 then
			...
			for b in own_bullets
				if abs(ty + 8 - b.y) < 12 and x >= b.x and x <= b.x + 10 then ' The bullet has hit an enemy.
				    ...
				    ' b_index = INDEX_OF(own_bullets, b)
...
' Updates spaceship
def update_spaceship(ship_type)
	if ship_type = 1 then
		spaceship = load_resource("spaceship_1.sprite")
		spaceship.play()
	endif
	if ship_type = 2 then
		spaceship = load_resource("spaceship_2.sprite")
		spaceship.play()
	endif
enddef
...
def pelter(delta)
	...
	for b in own_bullets
		b.x = b.x + b.vx * delta
		if b.color = nil then
		    b.color = rgba(149, 202, 60)' rnd_color()
		endif
		line b.x, b.y, b.x + 2, b.y, 1, b.color
	next
...
def galaxy(delta, sp)
	...
	smallFont.puts("SCORE:", 1, 2, 6)
	' text 47, 1, score, WHITE
	s_score = str(score)
	smallFont.puts(s_score, 30, 2, 6)
	' text 80, 1, "HI:", WHITE
	smallFont.puts("HIT:", 80, 2, 6)
	' text 104, 1, highscore
	' print highscore
	s_highscore = str(highscore)
	smallFont.puts(s_highscore, 97, 2, 6)
enddef
...
def move_up(delta)
	sy = sy + delta * 50
	if sy >= 112 then sy = 112
enddef

def move_right(delta)
	sx = sx + delta * 50
	if sx >=150 then sx = 150
enddef
...
def battle(delta)
	...
	if btnp(4) then
		if len(own_bullets) < OWN_BULLETS_MAX_COUNT then
			b1 = gen_bullet(sx,sy - 5)
			b2 = gen_bullet(sx,sy + 3)
			push(own_bullets, b1)
			push(own_bullets, b2)
